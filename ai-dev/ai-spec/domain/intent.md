...
### AG-11: 执行快照 SQL 展示脱敏
执行快照（`execution_snapshot`）中的 SQL 在"重放"UI 展示时，必须按当前用户权限脱敏，至少隐藏数据库连接串和密码类参数。存储侧保留完整 SQL 供审计。

### AG-12: 快照结果预览权限过滤
快照中冻结的结果预览数据，展示时必须经过当前用户的行级/列级权限过滤。不能因为"是历史数据"就跳过权限检查。

### AG-13: Prometheus 指标 tag 禁止高基数
Prometheus 指标 tag 禁止使用高基数字段（templateId / userId / SQL 文本），只允许低基数枚举（status / triggerType / channelType）。防止时序膨胀导致 Prometheus OOM。

### AG-14: 调度并发租户级上限
每租户同时执行的调度任务不超过 5 个（可配置）。防止单租户慢查询独占 Quartz 线程池，影响其他租户。
...
| execution_snapshot 中的 SQL | 中敏 | 存储时保留完整（审计需要），UI 展示时按权限脱敏连接信息 |
| 快照结果预览数据 | 中~高敏 | 存储时仅保留前 20 行摘要，展示时经过列级权限过滤 |
| 飞书 Webhook URL | 高敏 | API 返回时脱敏为 `https://open.feishu.cn/...****`，日志不打印完整 URL |
| 飞书 Webhook Secret | 极高敏 | 加密存储，日志禁止打印，API 返回时置空 |
