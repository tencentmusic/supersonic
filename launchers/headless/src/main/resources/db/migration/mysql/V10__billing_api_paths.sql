...
VALUES
-- 租户订阅管理权限 (平台管理员)
('API_ADMIN_TENANT_SUBSCRIPTION', '租户订阅管理', '管理租户的订阅分配', 'PLATFORM', 'API', '/api/v1/tenants/*/subscription', 1, 'system'),
-- 自助订阅权限 (租户用户)
('API_MY_SUBSCRIPTION', '我的订阅', '查看和管理自己的订阅', 'TENANT', 'API', '/api/v1/my-subscription', 1, 'system')
AS new_val ON DUPLICATE KEY UPDATE `path`=new_val.`path`, `description`=new_val.`description`;
