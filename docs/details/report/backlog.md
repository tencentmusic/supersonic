...
- 飞书开放平台 IM 能力，按绑定用户 `open_id` 等向私聊或指定会话投递定时报表结果
- 依赖用户与飞书账号绑定、权限与外部分发策略；与问数链路中 `FeishuMessageSender` 等能力的复用关系需在专项方案中定义

产品侧占位见 [运营数据工作台产品设计说明书 §11.3](../../product/运营数据工作台产品设计说明书.md#113-投递与触达增强占位)。
