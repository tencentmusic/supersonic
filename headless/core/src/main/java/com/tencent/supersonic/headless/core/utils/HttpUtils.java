...
public class HttpUtils {
    ...
    private static final OkHttpClient client = new OkHttpClient.Builder().readTimeout(3, TimeUnit.MINUTES)
    ...
    public static <T> T get(
            String url, Map<String, String> headers, Map<String, Object> params, Class<T> classOfT
    ) throws IOException {
    ...
    public static <T> T get(String url, Map<String, String> headers, TypeReference<T> type) throws IOException {
        return doRequest(headerBuilder(url, headers).build(), type);
    }

    public static <T> T get(
            String url, Map<String, String> headers, Map<String, Object> params, TypeReference<T> type
    ) throws IOException {
    ...
    public static <T> T post(
            String url, Object body, Map<String, String> headers, Class<T> classOfT
    ) throws IOException {
        return doRequest(postRequest(url, body, headers), classOfT);
    }

    public static <T> T post(
            String url, Object body, Map<String, String> headers, TypeReference<T> type
    ) throws IOException {
    ...
    private static <T> T doRequest(Request request, Class<T> classOfT) throws IOException {
        return JsonUtils.fromJson(doRequest(request), classOfT);
    }

    private static <T> T doRequest(Request request, TypeReference<T> type) throws IOException {
        return JsonUtils.fromJson(doRequest(request), type);
    }
    ...
    private static String doRequest(Request request) throws IOException {
        ...
        try {
            ...
            logger.info("begin to request : {}, execute costs(ms) : {}",
                    request.url(), System.currentTimeMillis() - beginTime);
        }
    }
    ...
    private static RequestBody buildRequestBody(Object body, Map<String, String> headers) {
        ...
        if (body instanceof String && ((String) body).contains("=")) {
            return RequestBody.create(MediaType.parse("application/x-www-form-urlencoded"), (String) body);
        }

        return RequestBody.create(MediaType.parse("application/json"), JsonUtils.toJsonString(body));
    }
