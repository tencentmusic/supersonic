...
public class JdbcDataSource {
    ...
    public DruidDataSource getDataSource(DatabaseResp database) throws RuntimeException {
        return getDataSource(database, null, null, null);
    }
    ...
    public DruidDataSource getDataSource(DatabaseResp database, String poolType,
        ...
        // Use base key when no pool type is specified; otherwise include pool type for isolation
        String key = poolType == null ? getDataSourceKey(database)
                : getDataSourceKeyWithPoolType(database, poolType);
        ...
        try {
            ...
            druidDataSource.setName(poolType == null ? name : name + "_" + poolType);
            ...
            druidDataSource
                    .setMinIdle(poolType == null ? minIdle : Math.min(minIdle, effectiveMaxActive));
            ...
            if (poolType != null) {
                log.info("Created {} pool for database {}: maxActive={}, maxWait={}", poolType,
                        name, effectiveMaxActive, effectiveMaxWait);
            }
