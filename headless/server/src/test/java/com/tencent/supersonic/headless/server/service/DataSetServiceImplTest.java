 ...
 *
 * <p>
 * MyBatis-Plus 的 lambda QueryWrapper 在脱离 Spring 上下文时无法生成 SQL（缺少 TableInfo 缓存）， 因此本测试用 mock mapper 在
 * selectList 桩内以 Java 模拟数据库 LIKE 语义（对 name / bizName 做 子串匹配），断言服务返回的列表被正确收窄：
 * <ul>
 * <li>name 为空 → 返回全量（受域/状态约束）；</li>
 * <li>name 非空 → 仅返回 name 或 bizName 包含关键字的记录；</li>
 * <li>命中既可能来自 name，也可能来自 bizName。</li>
 * </ul>
 * 注：生产代码使用 {@code lambda().like(...)} 生成标准 SQL LIKE，对 Postgres/Mysql 通用； 大小写敏感性取决于数据库 collation（与现有
 * AppServiceImpl/TermServiceImpl 一致）。
 */
class DataSetServiceImplTest {

    private static final List<DataSetDO> ALL = Arrays.asList(dataSet(1L, "销售数据集", "sales_daily", 1),
            dataSet(2L, "用户数据集", "user_profile", 1), dataSet(3L, "财务数据集", "finance", 2),
    ...
    void getDataSetList_withoutName_returnsAll() {
        ...
        Mockito.when(mapper.selectList(any()))
                .thenAnswer(inv -> simulateLike(inv.getArgument(0), null));

        List<DataSetResp> result =
                spyService(mapper).getDataSetList(10L, Arrays.asList(1, 2), null);
    ...
    void getDataSetList_withName_matchesByName() {
        DataSetDOMapper mapper = Mockito.mock(DataSetDOMapper.class);
        Mockito.when(mapper.selectList(any()))
                .thenAnswer(inv -> simulateLike(inv.getArgument(0), "销售"));

        List<DataSetResp> result =
                spyService(mapper).getDataSetList(10L, Arrays.asList(1, 2), "销售");
    ...
    void getDataSetList_withName_matchesByBizName() {
        DataSetDOMapper mapper = Mockito.mock(DataSetDOMapper.class);
        Mockito.when(mapper.selectList(any()))
                .thenAnswer(inv -> simulateLike(inv.getArgument(0), "user"));

        List<DataSetResp> result =
                spyService(mapper).getDataSetList(10L, Arrays.asList(1, 2), "user");
    ...
    void getDataSetList_withName_noMatch_returnsEmpty() {
        DataSetDOMapper mapper = Mockito.mock(DataSetDOMapper.class);
        Mockito.when(mapper.selectList(any()))
                .thenAnswer(inv -> simulateLike(inv.getArgument(0), "不存在的关键字"));

        List<DataSetResp> result =
                spyService(mapper).getDataSetList(10L, Arrays.asList(1, 2), "不存在的关键字");
